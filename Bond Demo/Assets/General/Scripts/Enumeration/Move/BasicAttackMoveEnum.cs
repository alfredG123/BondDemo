...
{
    public static BasicAttackMove Tackle = new BasicAttackMove(0, "Tackcle", "Deal damage that equals 40% of the attack. Gain 10.", 1, TypeAttribute.Normal, TypeTargetSelection.SingleTarget, 4f, 1, 10);

    public BasicAttackMove(int value, string name, string description, int priority, TypeAttribute attribute, TypeTargetSelection target_selection_type, float power, float accuracy, int energy_gain, bool has_secondary_effect = false, TypeFieldEffect field_effect_type = TypeFieldEffect.None, TypeLastingStatusEffect lasting_status_effect_type = TypeLastingStatusEffect.None, TypeTemporaryStatusEffect temporary_status_effect_type = TypeTemporaryStatusEffect.None, float secondary_effect_chance = 0)
